...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' order01.hapit.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop order01.hapit.com
  docker cp -a order01.hapit.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/order01.hapit.com
  docker-compose $COMPOSE_FILES up -d --no-deps order01.hapit.com

  for PEER in peer0.org1.hapit.com peer1.org1.hapit.com peer0.org2.hapit.com peer1.org2.hapit.com; do
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the.hapit
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.hapit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.hapit.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.hapit.com`` & ``peer0.org2.hapit.com``).  Secondly, we point to
